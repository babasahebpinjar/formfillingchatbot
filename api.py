...
9. Don't ask the information which is already given.
10. Make sure the information are in the format mentioned in the field description and let user know this
...
mongo_uri = "mongodb+srv://chatbotuser:novell1320@aetiustestdb.xi8oe.mongodb.net/"
db_name = "chatbotdb"
...
def update_customer_table(session_id: object, data: object) -> object:
    try:
        result = collection.update_one(
            {"_id": session_id},
            {"$set": {"name": data['name'], "city": data['city'], "email": data['email'],"emiratesid": data['emiratesid'],"companyname":data['companyname']}},
            upsert=True
        )
...
def check_details_from_db(session_id: object) -> object:
    try:
        existing_row = collection.find_one({"_id": session_id})

        if existing_row:
            # If a document with the session_id exists, return the document
            return existing_row
        else:
            # If the session_id doesn't exist, create a new document
            result = collection.insert_one({"_id": session_id})
            new_row = collection.find_one({"_id": session_id})
            return new_row
    except ConnectionError as e:
        print(f"Error connecting to MongoDB: {e}")
        return None
...
class PersonalDetails(BaseModel):
    ...
    emiratesid: str = Field(
        None,
        description="an Emirates ID is the ID given in Dubai and unique to each person and should be in \
                        123-456-789 format",
    )
    companyname: str = Field(
        None,
        description="its the company for which the user is working for",
    )
